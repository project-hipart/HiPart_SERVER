...
router.get('/creator', authUtil.isLoggedin, async (req, res) => {
...
router.get('/editor', authUtil.isLoggedin, async (req, res) => {
    const SelectUserQuery = "SELECT user_idx, user_nickname, user_img, user_type FROM user WHERE user_idx = ?"
    const SelectUserResult = await db.queryParam_Arr(SelectUserQuery, [req.decoded.idx]);
...
router.get('/translator', authUtil.isLoggedin, async (req, res) => {
    const SelectUserQuery = "SELECT user_idx, user_nickname, user_img, user_type FROM user WHERE user_idx = ?"
    const SelectUserResult = await db.queryParam_Arr(SelectUserQuery, [req.decoded.idx]);
...
router.get('/etc', authUtil.isLoggedin, async (req, res) => {
    const SelectUserQuery = "SELECT user_idx, user_nickname, user_img, user_type FROM user WHERE user_idx = ?"
    const SelectUserResult = await db.queryParam_Arr(SelectUserQuery, [req.decoded.idx]);
