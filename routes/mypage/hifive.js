...
router.get('/', authUtil.isLoggedin, async (req, res) => {
    ...
    if (!selectResult) {
        ...
        for (let i = 0; i < selectResult.length; i++) {
            ...
            const SelectPickResult = await db.queryParam_Arr(SelectPickQuery, [req.decoded.idx, selectResult[i].hifive_to]);
            ...
            const selectUserResult = await db.queryParam_Arr(selectUserQuery, [selectResult[i].hifive_to]);

            item.info.push(selectUserResult[0]);
            resData.push(item);
        }
        res.status(200).send(defaultRes.successTrue(statusCode.OK, "하이파이브 리스트 조회 성공", resData));
    }
});
