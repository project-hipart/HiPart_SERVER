...
router.post('/', upload.single('user_img'), async (req, res) => {
    ...
    const signupQuery = 'INSERT INTO user (user_email,user_nickname,user_img,user_pw,user_number,salt,user_type,point) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    ...
    if (selectIdResult[0] == null) {// 이미 존재
        ...
        const signupResult = await db.queryParam_Arr(signupQuery, [req.body.user_email, req.body.user_nickname, req.file.location, hashedPw.toString('base64'), req.body.user_number, salt, req.body.user_type, 30]);
